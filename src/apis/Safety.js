...
export const FetchSafeties = async () => {
  try {
    const response = await axios.get("/safety/fetch", {
      headers: {
...
export const CreateSafety = async (itemName, status) => {
  try {
...
export const DeleteSafety = async (safetyId) => {
  const token = localStorage.getItem("token"); // Retrieve the token from localStorage
  const response = await axios.delete("/safety/remove", {
    data: {
      data: {
        safetyId: safetyId,
      },
